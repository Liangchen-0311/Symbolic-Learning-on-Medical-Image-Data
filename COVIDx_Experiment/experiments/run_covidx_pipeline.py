#!/usr/bin/env python3
"""
COVIDx CT-3A chest CT Symbolic Feature Discovery Pipeline (grayscale).
...
from src.data.covidx_loader import (
    COVIDXDataset, COVIDXDataModule, build_covidx_data_batch,
    build_covidx_superclass_mapping, COVIDX_NAMES, COVIDX_FULL_NAMES,
    COVIDX_SUPERCLASS, SUPERCLASS_NAMES,
)
...
def step0_validate_dataset(config, device):
    print(f"\n{'='*70}")
    print(f"  STEP 0: Validate COVIDx CT-3A Dataset")
    print(f"{'='*70}")
    ...
    print(f"  Label names: {[COVIDX_NAMES[l] for l in batch['labels'].tolist()]}")
...
def step1_phase1(config, device):
    print(f"\n{'='*70}")
    print(f"  STEP 1: Phase 1 — Symbolic Feature Discovery (grayscale chest CT + early stop)")
    print(f"{'='*70}")
    ...
    for bank_id, bank_cfg in enumerate(bank_configs):
        ...
        if config['dataset_options']['num_classes'] >= 3 and \
            ...
            superclass_groups, superclass_names = build_covidx_superclass_mapping()
            class_to_super = {}
...
def step3_validate(config, device):
    ...
    seed = config['dataset_options'].get('split_seed', 42)
    train_ds = COVIDXDataset(data_dir, split='train', resolution=res,
...
def step6_report(config, device):
    ...
    lines = []
    lines.append("=" * 70)
    lines.append("COVIDX CT-3A SYMBOLIC FEATURE INTERPRETABILITY REPORT")
    lines.append("=" * 70)
    lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description='COVIDx CT-3A Symbolic Pipeline')
    ...
    print(f"\n{'='*70}")
    print(f"  COVIDx CT-3A Symbolic Feature Discovery Pipeline")
