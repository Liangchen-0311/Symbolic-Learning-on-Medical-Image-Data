...
class TensorTokenVocabulary:
    ...
    def __init__(self, exclude_operators=None, extra_terminals=None, terminal_tokens=None):
        # Special tokens
        ...
        # Terminals. Default: RGB + grayscale + HSV + color ratios + opponent
        # channels (HAM10000). Datasets with different modalities (e.g. grayscale
        # MRI) inject their own terminal name list via `terminal_tokens`.
        if terminal_tokens is None:
            terminal_tokens = ['I_R', 'I_G', 'I_B', 'I_GRAY', 'I_H', 'I_S',
                               'I_r', 'I_g', 'I_RG', 'I_BY']
        else:
            terminal_tokens = list(terminal_tokens)
            print(f"  [Vocab] Custom terminals: {terminal_tokens}")
...
class TensorVSREnvironmentLargeBank(gym.Env):
    ...
    def __init__(
        ...
        # --- Residual / boosting reward (functional-gradient on formula search) ---
        self.residual_refresh = strategy_cfg.get('residual_refresh', 50)
        self._eval_data = None       # fixed reference terminals dict (for alignment)
        self._eval_labels = None
        self._residual = None        # [N, C] = onehot(y) - ensemble softmax
        self._episodes_since_refresh = 10**9
        self._residual_seed_strs = []
        _seed_path = strategy_cfg.get('residual_seed_path', None)
        if _seed_path and os.path.exists(_seed_path):
            import json as _json
            try:
                _seed = _json.load(open(_seed_path))
                self._residual_seed_strs = [f['str'] for f in _seed]
                print(f"  [Residual] seeded with {len(self._residual_seed_strs)} accumulated formulas")
            except Exception:
                pass
        ...
        # v3: unified resolution — use single 'resolution' key
        self.eval_resolution = dataset_options.get('resolution', None)
        ...
        # Datasets with a non-RGB modality (e.g. grayscale MRI) may inject a
        # custom terminal name list via config['terminal_tokens']; subclasses
        # are responsible for producing matching channels in get_data_batch.
        self.terminal_tokens = config.get('terminal_tokens', None)
        self.vocabulary = TensorTokenVocabulary(
            ...
            terminal_tokens=self.terminal_tokens,
        )
    ...
    def _cache_validation_set(self) -> None:
        """Cache a class-balanced subset of images for RL reward evaluation.

        v3: At 600x600 resolution, caching all images would exceed GPU memory.
        We cache at most `cache_max_images` images (default 512) with balanced
        class distribution, and keep them on CPU, moving only the needed batch
        to GPU on demand.

        Uses two-pass streaming: first pass counts per-class, second pass
        samples up to per_class quota from each class — never holds all
        images in memory at once.
        """
        cache_max_images = getattr(self, 'cache_max_images', 512)

        # Pass 1: count images per class
        class_counts = {}
        for _, labels in self.data_loader:
            for cls_id in labels.unique():
                cls_id = cls_id.item()
                class_counts[cls_id] = class_counts.get(cls_id, 0) + (labels == cls_id).sum().item()
        num_classes = len(class_counts)
        if num_classes == 0:
            raise RuntimeError("Empty validation cache")

        # Balanced allocation: equal images per class
        per_class = cache_max_images // num_classes
        # Track how many we've collected per class
        collected = {c: 0 for c in class_counts}
        selected_images = []
        selected_labels = []

        # Pass 2: sample up to per_class from each class
        for images, labels in self.data_loader:
            batch_selected_idx = []
            for i in range(images.shape[0]):
                cls_id = labels[i].item()
                if collected[cls_id] < per_class:
                    batch_selected_idx.append(i)
                    collected[cls_id] += 1

            if batch_selected_idx:
                idx_tensor = torch.tensor(batch_selected_idx)
                selected_images.append(images[idx_tensor])
                selected_labels.append(labels[idx_tensor])

            # Check if all classes are filled
            if all(collected[c] >= per_class for c in class_counts):
                break

        # v3: Keep on CPU to save GPU memory; move to device in get_data_batch
        self.cached_images = torch.cat(selected_images, dim=0)  # stays on CPU
        self.cached_labels = torch.cat(selected_labels, dim=0)  # stays on CPU

        # Print class distribution
        label_counts = torch.bincount(self.cached_labels, minlength=num_classes)
        dist_str = ", ".join(f"c{c}:{label_counts[c].item()}" for c in range(num_classes) if label_counts[c] > 0)
        print(f"Cached {self.cached_images.shape[0]} images (CPU, balanced): [{dist_str}]")
    ...
    def get_data_batch(
        ...
        # Sample from cached data (CPU → GPU on demand)
    ...
    # ---- Residual reward helpers ----
    def _exec_eval(self, tokens):
        """Plain RPN eval over the fixed eval terminals (no subexpr cache)."""
        stack = []
        for t in tokens:
            name = self.vocabulary.decode(t) if isinstance(t, int) else t
            if name in ("START", "END", "PAD"):
                continue
            if name in self._eval_data:
                stack.append(self._eval_data[name])
            elif name in TENSOR_OPERATORS:
                fn, arity, _ = TENSOR_OPERATORS[name]
                if len(stack) < arity:
                    return None
                ops = [stack.pop() for _ in range(arity)]
                ops.reverse()
                try:
                    stack.append(fn(*ops))
                except Exception:
    ...
    def _feat_on_eval(self, tokens):
        out = self._exec_eval(tokens)
        if out is None or out.dim() < 1:
            return None
        out = torch.nan_to_num(out, nan=0.0, posinf=1e4, neginf=-1e4)
        if out.dim() > 1:
            out = out.reshape(out.shape[0], -1).mean(dim=1)
        if out.shape[0] != self._eval_labels.shape[0] or out.std() < 1e-8:
            return None
        return (out - out.mean()) / (out.std() + 1e-8)

    def _ensure_eval_data(self):
        if self._eval_data is None:
            n = min(self.eval_batch_size, self.cached_images.size(0))
            self._eval_data, self._eval_labels = self.get_data_batch(
                batch_size=n, resolution=self.eval_resolution)

    def _refresh_residual(self):
        self._ensure_eval_data()
        y = self._eval_labels.long()
        C = self.num_classes
        onehot = torch.zeros(y.shape[0], C, device=y.device)
        onehot[torch.arange(y.shape[0], device=y.device), y] = 1.0
        feats = []
        # Ensemble = a random sample across the FULL accumulated seed (so every
        # previous boosting round is represented, not just the first slice) plus
        # the current bank. random sample avoids the ordering bug where [:N] only
        # picked the baseline formulas.
        seed = list(self._residual_seed_strs)
        if len(seed) > 700:
            idx = np.random.choice(len(seed), 700, replace=False)
            seed = [seed[i] for i in idx]
        ens_strs = seed + list(self.feature_bank.formula_strs)[:200]
        for fs in ens_strs:
            f = self._feat_on_eval(fs.split())
            if f is not None:
                feats.append(f)
        if len(feats) < 2:
            self._residual = (onehot - 1.0 / C).detach()   # cold start
            return
        X = torch.stack(feats, dim=1)
        model = torch.nn.Linear(X.shape[1], C).to(X.device)
        ...
        self._residual = (onehot - p).detach()

    def _residual_reward(self, tokens):
        if self._residual is None or self._episodes_since_refresh >= self.residual_refresh:
            self._refresh_residual()
            self._episodes_since_refresh = 0
        f = self._feat_on_eval(tokens)
        if f is None:
            return 0.0
        R = self._residual
        Rn = (R - R.mean(0, keepdim=True)) / (R.std(0, keepdim=True) + 1e-8)
        corr = (Rn * f.unsqueeze(1)).mean(0).abs()   # [C]
        return float(corr.max().item())
    ...
    def _calculate_reward(self) -> Tuple[float, Dict]:
        ...
        try:
            ...
            # v2: Reject zero-variance features (e.g. tophat on smooth signals)
            if feature_tensor.std() < 1e-8:
                return -0.8, {"valid": False, "reason": "zero_variance_output"}
            ...
            elif self.reward_type == 'residual':
                self._episodes_since_refresh += 1
                reward_score = self._residual_reward(expression_tokens)
                accuracy = reward_score          # bank admission gate
                top5_acc = 0.0
            else:
            ...
            spatial_ops = {'blur', 'blur_7x7', 'edge_x', 'edge_y', 'dilate', 'laplacian', 'normalize',
                          'opening', 'closing', 'high_freq', 'low_freq'}
            ...
            # v2: Low-variance penalty — penalize features with poor discriminability
            raw_std = formula_output.reshape(formula_output.shape[0], -1).std()
            low_var_penalty = 0.0
            if raw_std < 0.01:
                low_var_penalty = 0.3
            elif raw_std < 0.05:
