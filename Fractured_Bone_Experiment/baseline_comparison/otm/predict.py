...
def _load_tm(model_dir):
    """Load TMClassifier from pickle. Handles both full-state and array-only formats."""
    ...
    if isinstance(state, dict) and state.get('format') == 'cuda_arrays_v1':
        # Array-only format (GPU-trained, CPU-rebuild): create CPU TM and inject arrays
        tm = TMClassifier(
        ...
        # Trigger initialization by a dummy fit, then overwrite learned arrays
        dim = state['dim']
