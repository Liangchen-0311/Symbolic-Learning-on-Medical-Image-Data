...
def step5_train_classifier(config, device):
    print(f"\n{'='*70}")
    print(f"  STEP 5: Train Classifier (v3 — No SMOTE + PCA + Anti-overfit)")
    print(f"{'='*70}")
    ...
    anova_preselect = min(500, combine_X_nc.shape[1])
    ...
    print(f"\n  --- Method 1: LR + ANOVA + PCA ---", flush=True)
    pca_dims = [30, 50, 80, 120]
    if n_components_99 < 120:
        pca_dims = [d for d in pca_dims if d <= n_components_99]
    C_values = [0.01, 0.1, 1.0, 10.0]
    best_cv1, best_K1, best_C1 = 0, None, None
    for n_comp in pca_dims:
        for C in C_values:
            ...
            if cv > best_cv1:
                best_cv1, best_K1, best_C1 = cv, n_comp, C
    print(f"  Best: PCA={best_K1}, C={best_C1}, cv_acc={best_cv1:.3f}")
    pipe_lr1 = Pipeline([
        ('pca', PCA(n_components=best_K1, random_state=42)),
        ('clf', LogisticRegression(C=best_C1, max_iter=5000, solver='lbfgs', class_weight='balanced')),
    ])
    ...
    results['lr_pca'] = {'acc': float(acc), 'auc': float(auc)}
    best_pipes['lr_pca'] = pipe_lr1
    cv_accs['lr_pca'] = best_cv1

    print(f"\n  --- Method 2: LR + ANOVA direct ---", flush=True)
    K_values = [30, 50, 80, 120, 200, 300]
    C_values2 = [0.01, 0.1, 1.0, 10.0]
    best_cv2, best_K2, best_C2 = 0, None, None
    for K in K_values:
        ...
        for C in C_values2:
            ...
            cv = _cv_eval(pipe, combine_anova, combine_y)
            if cv > best_cv2:
                best_cv2, best_K2, best_C2 = cv, K, C
    print(f"  Best: K={best_K2}, C={best_C2}, cv_acc={best_cv2:.3f}")
    pipe_lr2 = Pipeline([
        ('select', SelectKBest(f_classif, k=best_K2)),
        ('scale', StandardScaler()),
        ('clf', LogisticRegression(C=best_C2, max_iter=5000, solver='lbfgs', class_weight='balanced')),
    ])
    pipe_lr2.fit(combine_anova, combine_y)
    preds = pipe_lr2.predict(test_anova)
    acc = accuracy_score(test_y, preds)
    auc = _calc_auc(pipe_lr2, test_anova, test_y)
    print(f"  Test: acc={acc:.3f}, AUC={auc:.3f}")
    results['lr_anova'] = {'acc': float(acc), 'auc': float(auc)}
    best_pipes['lr_anova'] = pipe_lr2
    cv_accs['lr_anova'] = best_cv2

    print(f"\n  --- Method 3: LR + MI (ANOVA pre-filter) ---", flush=True)
    mi_K_values = [30, 50, 80, 120]
    mi_C_values = [0.1, 1.0, 10.0]
    best_cv3, best_K3, best_C3 = 0, None, None
    for K in mi_K_values:
        if K > anova_preselect:
            continue
        for C in mi_C_values:
            pipe = Pipeline([
            ...
            cv = _cv_eval(pipe, combine_anova, combine_y)
            if cv > best_cv3:
                best_cv3, best_K3, best_C3 = cv, K, C
    print(f"  Best: K={best_K3}, C={best_C3}, cv_acc={best_cv3:.3f}")
    pipe_lr3 = Pipeline([
        ('select', SelectKBest(mutual_info_classif, k=best_K3)),
        ('scale', StandardScaler()),
        ('clf', LogisticRegression(C=best_C3, max_iter=5000, solver='lbfgs', class_weight='balanced')),
    ])
    pipe_lr3.fit(combine_anova, combine_y)
    preds = pipe_lr3.predict(test_anova)
    acc = accuracy_score(test_y, preds)
    auc = _calc_auc(pipe_lr3, test_anova, test_y)
    print(f"  Test: acc={acc:.3f}, AUC={auc:.3f}")
    results['lr_mi'] = {'acc': float(acc), 'auc': float(auc)}
    best_pipes['lr_mi'] = pipe_lr3
    cv_accs['lr_mi'] = best_cv3

    print(f"\n  --- Method 4: RandomForest ---", flush=True)
    rf_configs = [(100, 5), (200, 5), (200, 10), (300, 10)]
    rf_K_values = [50, 100, 200]
    best_rf_cv, best_rf_cfg, best_rf_K = 0, None, None
    for K in rf_K_values:
        if K > anova_preselect:
            continue
        for n_est, min_s in rf_configs:
            pipe = Pipeline([
                ('select', SelectKBest(f_classif, k=K)),
                ('scale', StandardScaler()),
                ('clf', RandomForestClassifier(n_estimators=n_est, min_samples_leaf=min_s,
                                               class_weight='balanced_subsample', random_state=42, n_jobs=-1)),
            ])
            cv = _cv_eval(pipe, combine_anova, combine_y)
            if cv > best_rf_cv:
                best_rf_cv, best_rf_cfg, best_rf_K = cv, (n_est, min_s), K
    print(f"  Best: n_est={best_rf_cfg[0]}, min_samples={best_rf_cfg[1]}, K={best_rf_K}, cv_acc={best_rf_cv:.3f}")
    pipe_rf = Pipeline([
        ('select', SelectKBest(f_classif, k=best_rf_K)),
        ('scale', StandardScaler()),
        ('clf', RandomForestClassifier(n_estimators=best_rf_cfg[0], min_samples_leaf=best_rf_cfg[1],
                                       class_weight='balanced_subsample', random_state=42, n_jobs=-1)),
    ])
    pipe_rf.fit(combine_anova, combine_y)
    preds = pipe_rf.predict(test_anova)
    acc = accuracy_score(test_y, preds)
    auc = _calc_auc(pipe_rf, test_anova, test_y)
    print(f"  Test: acc={acc:.3f}, AUC={auc:.3f}")
    results['rf'] = {'acc': float(acc), 'auc': float(auc)}
    best_pipes['rf'] = pipe_rf
    cv_accs['rf'] = best_rf_cv
    ...
    for method, res in results.items():
        ...
        print(f"    {method:15s}: acc={res['acc']:.3f}, AUC={res['auc']:.3f}{cv_info} gap={gap:.3f}{marker}")
    ...
    for c in range(num_classes):
        ...
        if mask.sum() > 0:
            ...
            per_class[name] = float((best_preds[mask] == c).mean())
