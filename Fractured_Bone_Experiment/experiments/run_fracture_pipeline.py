...
def step5_train_classifier(config, device):
    ...
    print(f"\n  --- Method 1: L2-LR + ANOVA ---", flush=True)
    lr_K_values = [100, 200, 300]
    lr_C_values = [0.1, 1.0, 10.0]
    best_cv1, best_K1, best_C1 = 0, None, None
    for K in lr_K_values:
    ...
    best_pipes['lr_l2'] = pipe_lr1
    cv_accs['lr_l2'] = best_cv1

    print(f"\n  --- Method 2: HistGradientBoosting + ANOVA ---", flush=True)
    hgb_K_values = [100, 200, 300]
    hgb_configs = [(100, 0.1, 3), (200, 0.1, 5), (200, 0.05, 3), (300, 0.1, 5)]
    best_hgb_cv, best_hgb_cfg, best_hgb_K = 0, None, None
    for K in hgb_K_values:
        ...
        for max_iter, lr, max_d in hgb_configs:
            ...
            cv = _cv_eval(pipe, combine_anova, combine_y)
            if cv > best_hgb_cv:
                best_hgb_cv, best_hgb_cfg, best_hgb_K = cv, (max_iter, lr, max_d), K
    print(f"  Best: max_iter={best_hgb_cfg[0]}, lr={best_hgb_cfg[1]}, max_depth={best_hgb_cfg[2]}, K={best_hgb_K}, cv_acc={best_hgb_cv:.3f}")
    pipe_hgb = Pipeline([
    ...
    pipe_hgb.fit(combine_anova, combine_y)
    preds = pipe_hgb.predict(test_anova)
    acc = accuracy_score(test_y, preds)
    auc = _calc_auc(pipe_hgb, test_anova, test_y)
    print(f"  Test: acc={acc:.3f}, AUC={auc:.3f}")
    results['hist_gradient_boosting'] = {'acc': float(acc), 'auc': float(auc)}
    best_pipes['hist_gradient_boosting'] = pipe_hgb
    cv_accs['hist_gradient_boosting'] = best_hgb_cv

    print(f"\n  --- Method 3: ExtraTrees + ANOVA ---", flush=True)
    et_K_values = [100, 200]
    et_configs = [(200, 5), (300, 5), (300, 10)]
    ...
    for mn, mpipe in best_pipes.items():
        joblib.dump({
            ...
            'scaler': scaler_for_pca,
            'anova_selector': anova_selector,
        }, version_dir / f'classifier_{mn}_v{version}.pkl')
