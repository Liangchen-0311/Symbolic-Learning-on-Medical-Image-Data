#!/usr/bin/env python3
"""
Brain Tumor MRI Symbolic Feature Discovery Pipeline (grayscale).
...
from src.data.brain_tumor_loader import (
    BrainTumorDataset, BrainTumorDataModule, build_brain_data_batch,
    build_brain_superclass_mapping, BRAIN_NAMES, BRAIN_FULL_NAMES,
    BRAIN_SUPERCLASS, SUPERCLASS_NAMES,
)
...
def step0_validate_dataset(config, device):
    print(f"\n{'='*70}")
    print(f"  STEP 0: Validate Brain Tumor MRI Dataset")
    print(f"{'='*70}")
    ...
    print(f"  Label names: {[BRAIN_NAMES[l] for l in batch['labels'].tolist()]}")
...
def step1_phase1(config, device):
    print(f"\n{'='*70}")
    print(f"  STEP 1: Phase 1 — Symbolic Feature Discovery (grayscale MRI + early stop)")
    print(f"{'='*70}")
    ...
    for bank_id, bank_cfg in enumerate(bank_configs):
        ...
        if config['dataset_options']['num_classes'] >= 3 and \
            ...
            superclass_groups, superclass_names = build_brain_superclass_mapping()
            class_to_super = {}
...
def step3_validate(config, device):
    ...
    seed = config['dataset_options'].get('split_seed', 42)
    train_ds = BrainTumorDataset(data_dir, split='train', resolution=res,
...
def step6_report(config, device):
    ...
    lines = []
    lines.append("=" * 70)
    lines.append("BRAIN TUMOR MRI SYMBOLIC FEATURE INTERPRETABILITY REPORT")
    lines.append("=" * 70)
    lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description='Brain Tumor MRI Symbolic Pipeline')
    ...
    print(f"\n{'='*70}")
    print(f"  Brain Tumor MRI Symbolic Feature Discovery Pipeline")
