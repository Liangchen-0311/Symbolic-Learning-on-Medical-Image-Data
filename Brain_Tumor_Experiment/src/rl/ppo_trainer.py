...
class PPOTrainer:
    ...
    def __init__(
        ...
        # Soft per-operator logit prior (favor/penalize specific operators)
        self.operator_bias_vec = None
    ...
    def set_operator_bias(self, bias_map, vocabulary):
        """Soft per-operator logit prior. bias_map: {op_name: bias} (+ favors,
        - penalizes). Added to logits during sampling, like binary_op_bias."""
        self.operator_bias_vec = None
        if bias_map and vocabulary is not None:
            vec = torch.zeros(len(vocabulary), device=self.device)
            n = 0
            for name, b in bias_map.items():
                if name in vocabulary.token_to_idx:
                    vec[vocabulary.token_to_idx[name]] = float(b)
                    n += 1
            if n > 0:
                self.operator_bias_vec = vec
                print(f"  [Bias] operator prior on {n} ops: {bias_map}")
    ...
    def _apply_binary_bias(self, action_mask: 'torch.Tensor') -> 'torch.Tensor':
        ...
        """
        bias = None
        if self.binary_op_bias > 0 and self._binary_op_indices is not None:
        ...
        # Add the soft per-operator prior (favored/penalized operators).
        if self.operator_bias_vec is not None:
            ob = self.operator_bias_vec.to(action_mask.device)
            bias = ob.clone() if bias is None else bias + ob
        return bias
