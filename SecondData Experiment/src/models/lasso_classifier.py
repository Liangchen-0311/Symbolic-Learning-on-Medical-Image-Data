...
def train_lasso_classifier(
    ...
    Args:
        ...
        lr: Learning rate
        device: Device
        use_class_weight: If True, weight classes inversely proportional to count

    Returns:
        accuracy: Balanced accuracy (per-class mean)
    ...
    # v3: Use class-weighted cross entropy for imbalanced datasets
    if use_class_weight:
    ...
    # Evaluate — balanced accuracy (per-class mean)
    model.eval()
    with torch.no_grad():
        outputs = model(features_tensor)
        preds = torch.argmax(outputs, dim=1)
        # v3: Balanced accuracy = mean of per-class accuracies
        per_class_acc = []
        for c in range(num_classes):
            mask = targets == c
            if mask.sum() > 0:
                per_class_acc.append((preds[mask] == c).float().mean().item())
        accuracy = sum(per_class_acc) / len(per_class_acc) if per_class_acc else 0.0
